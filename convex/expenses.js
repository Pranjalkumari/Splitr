  ...
  handler: async (ctx, args) => {
    ...
    const results = await ctx.db
      ...
      .withSearchIndex("search_description", (q) =>
        q.search("description", args.query).eq("paidByUserId", user._id)
      )
      .collect();

    // If friendId provided, filter by that friend's involvement
    if (args.friendId) {
      return results.filter(e => 
        e.splits.some(s => s.userId === args.friendId)
      );
    }
